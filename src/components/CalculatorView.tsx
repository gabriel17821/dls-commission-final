...
export const CalculatorView = ({
  ...
  useEffect(() => {
    if (lastInvoice && !toastShownRef.current && lastInvoice.ncf) {
      try {
        ...
        const timeLabel = format(creationDate, 'h:mma', { locale: es }).toLowerCase().replace(/\s/g, ''); 

        toast.custom((t) => (
          // [MODIFICADO] max-w-xl para evitar el estrechamiento
  ...
  const handleContinueStep2 = () => {
    // [SOLUCIÓN DEL CRASH/PANTALLA BLANCA]
    // El crash ocurre al intentar renderizar ClientSelector en el DOM con props incorrectas
    // O al hacer una transición de estado inestable. El fix en ClientSelector.tsx lo estabiliza.
    if (selectedClient) setStep2Complete(true);
    // Si no hay cliente seleccionado, permitimos pasar de la etapa 2, pero sin el cliente asociado
    // Esto es un comportamiento razonable si el cliente es opcional en la BD. 
    // Basado en el código original, ClientSelector hace que el cliente sea un requisito para continuar, 
    // pero la data flow permite null. Asumo que el objetivo es forzar una selección aquí para la UX.
    // Usaremos el chequeo original.
  };
